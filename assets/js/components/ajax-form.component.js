  ...
  mounted: async function () {
    ...
    if (this.formRules) {
      ...
      for (const fieldName in this.formRules) {
        for (const ruleName in this.formRules[fieldName]) {
          if (_.contains(SUPPORTED_RULES, ruleName)) {
            ...
            const kebabRules = _.map(_.clone(SUPPORTED_RULES), (ruleName) => _.kebabCase(ruleName))
            const lowerCaseRules = _.map(_.clone(SUPPORTED_RULES), (ruleName) => ruleName.toLowerCase())
            const ruleIdx = (
    ...
    _submit: async function () {
      ...
      if (this.handleParsing) {
        ...
        for (const fieldName in this.formRules) {
          ...
          for (const ruleName in this.formRules[fieldName]) {
            const ruleRhs = this.formRules[fieldName][ruleName]
            let violation

            const isFieldValuePresent = (
