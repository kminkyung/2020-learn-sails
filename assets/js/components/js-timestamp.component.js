...
parasails.registerComponent('jsTimestamp', {
  ...
  methods: {
    ...
    _formatTimeago: function () {
      ...
      const timeDifference = Math.abs(now - this.at)
      ...
      if (timeDifference < 1000 * 60 * 60 * 24) {
        ...
        const includeTime = !this.short && timeDifference < 1000 * 60 * 60 * 24 * 31

        // If the timestamp is from a different year, we'll include the year
        // in the formatted timestamp.
        const includeYear = moment(now).format('YYYY') !== moment(this.at).format('YYYY')
