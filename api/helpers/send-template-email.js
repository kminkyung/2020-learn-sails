module.exports = {
  ...
  fn: async function ({ template, templateData, to, toName, subject, from, fromName, layout, ensureAck, bcc, attachments }) {
    ...
    if (dontActuallySend) {
      ...
      const subjectLinePrefix = sails.config.environment === 'production' ? '' : sails.config.environment === 'staging' ? '[FROM STAGING] ' : '[FROM LOCALHOST] '
      const messageData = {
      ...
      const deferred = sails.helpers.sendgrid.sendHtmlEmail.with(messageData)
      if (ensureAck) {
        await deferred
      } else {
