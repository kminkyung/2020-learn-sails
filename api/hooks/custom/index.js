...
module.exports = function defineCustomHook (sails) {
  return {
    ...
    initialize: async function () {
      ...
      const IMPORTANT_STRIPE_CONFIG = ['stripeSecret', 'stripePublishableKey']
      const IMPORTANT_SENDGRID_CONFIG = ['sendgridSecret', 'internalEmailAddress']
      const isMissingStripeConfig = _.difference(IMPORTANT_STRIPE_CONFIG, Object.keys(sails.config.custom)).length > 0
      const isMissingSendgridConfig = _.difference(IMPORTANT_SENDGRID_CONFIG, Object.keys(sails.config.custom)).length > 0

      if (isMissingStripeConfig || isMissingSendgridConfig) {
        const missingFeatureText = isMissingStripeConfig && isMissingSendgridConfig ? 'billing and email' : isMissingStripeConfig ? 'billing' : 'email'
        let suffix = ''
